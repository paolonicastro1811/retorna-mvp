...
export function SettingsPage() {
  ...
  if (!restaurant) return <div className="text-center py-20 text-red-500 text-sm">Erro</div>
    ...
    <div className="max-w-2xl">
      ...
      <section className="mb-8">
        <h2 className="text-base font-bold text-gray-900 mb-3 flex items-center gap-2">
          <span className="w-5 h-5 bg-[#25D366] rounded-full flex items-center justify-center text-white text-xs font-bold">1</span>
          Dados do Restaurante
        </h2>
        <div className="space-y-3 bg-gray-50 rounded-xl p-4">
          <div>
            <label className="block text-xs font-medium text-gray-600 mb-1">Nome do restaurante</label>
            <input type="text" value={name} onChange={e => setName(e.target.value)}
              className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-[#25D366] focus:border-transparent" />
          </div>
          <div>
          ...
          <div>
            <label className="block text-xs font-medium text-gray-600 mb-1">Fuso horario</label>
            <input type="text" value={timezone} onChange={e => setTimezone(e.target.value)}
              className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-[#25D366] focus:border-transparent" />
          </div>
          <button onClick={handleSave} disabled={saving || !name.trim()}
            className="w-full bg-[#25D366] text-white py-2 rounded-lg font-semibold text-sm hover:bg-[#1DA851] disabled:opacity-50 transition-colors">
            {saving ? 'Salvando...' : 'Salvar'}
          </button>
          {saved && <div className="bg-green-50 border border-green-200 text-green-800 rounded-lg p-2 text-xs">Salvo!</div>}
        </div>
      </section>
      ...
      <section className="mb-8">
        <h2 className="text-base font-bold text-gray-900 mb-3 flex items-center gap-2">
          <span className="w-5 h-5 bg-[#25D366] rounded-full flex items-center justify-center text-white text-xs font-bold">2</span>
          Programa de Fidelidade
        </h2>

        {/* Tier visualization */}
        <div className="bg-gray-50 rounded-xl p-4 mb-4">
          <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3">Niveis de fidelidade</p>
            ...
            {[
              ...
              <div key={t.tier} className="flex-1 text-center">
                <div className={`${t.color} rounded-xl p-2 mx-0.5`} style={{ minHeight: `${40 + i * 20}px` }}>
                  ...
                  <p className="text-[11px] text-gray-600">{t.visits}+ visitas</p>
                  <p className="text-[11px] font-semibold text-[#25D366]">{t.discount} desc.</p>
                </div>
              </div>
            ))}
          </div>
        ...
        {/* Config: tiers */}
        <div className="bg-gray-50 rounded-xl p-4 mb-4">
          <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3">Configurar niveis</p>
          <div className="grid grid-cols-3 gap-3">
            {[
              ...
              <div key={t.label} className="bg-white rounded-lg p-2.5 border border-gray-200">
                <p className="text-xs font-semibold text-gray-700 mb-2">{t.label}</p>
                <label className="text-[11px] text-gray-500">Visitas minimas</label>
                <input type="number" min="1" value={loyaltyConfig[t.visitKey]}
                  ...
                  className="w-full border border-gray-200 rounded px-2 py-1 text-sm mb-1.5 focus:outline-none focus:ring-1 focus:ring-[#25D366]" />
                <label className="text-[11px] text-gray-500">Desconto (%)</label>
          ...
          <div className="grid grid-cols-2 gap-3 mt-3">
            <div>
              <label className="text-[11px] text-gray-500">Retorna apos (dias sem visita)</label>
            ...
            <div>
              <label className="text-[11px] text-gray-500">Streak: visitas em {loyaltyConfig.streakWindowDays} dias</label>
          ...
          <button onClick={handleSaveLoyalty} disabled={savingLoyalty}
            className="w-full mt-3 bg-[#25D366] text-white py-2 rounded-lg font-semibold text-sm hover:bg-[#1DA851] disabled:opacity-50 transition-colors">
            {savingLoyalty ? 'Salvando...' : 'Salvar configuracao'}
          </button>
          {savedLoyalty && <div className="mt-2 bg-green-50 border border-green-200 text-green-800 rounded-lg p-2 text-xs">Configuracao salva!</div>}
        </div>
      ...
      <section className="mb-8">
        <h2 className="text-base font-bold text-gray-900 mb-3 flex items-center gap-2">
          <span className="w-5 h-5 bg-[#25D366] rounded-full flex items-center justify-center text-white text-xs font-bold">3</span>
          Mensagens Automaticas
        </h2>
        <div className="bg-gray-50 rounded-xl p-4">
          <p className="text-[11px] text-gray-400 mb-3">Ative ou desative as mensagens que o sistema envia automaticamente via WhatsApp.</p>
              ...
              <div key={tpl.id} className={`border rounded-xl p-3 transition-all ${tpl.isActive ? 'border-[#25D366] bg-white' : 'border-gray-200 bg-gray-100/50'}`}>
                <div className="flex items-center justify-between">
                  <div className="flex-1">
                    ...
                    <span className={`ml-2 text-[11px] px-1.5 py-0.5 rounded-full font-medium ${tpl.isActive ? 'bg-green-100 text-green-700' : 'bg-gray-200 text-gray-500'}`}>
                ...
                {tpl.isActive && (
                  <div className="mt-2 bg-[#DCF8C6] rounded-lg rounded-tl-none p-2 text-xs text-gray-800 leading-relaxed whitespace-pre-wrap max-w-[85%]">
                    {tpl.body
      ...
      {isPlanB && (
        <section className="mb-8">
          <h2 className="text-base font-bold text-gray-900 mb-3 flex items-center gap-2">
            <span className="w-5 h-5 bg-[#25D366] rounded-full flex items-center justify-center text-white text-xs font-bold">4</span>
            Horarios
          </h2>
          <div className="bg-gray-50 rounded-xl p-4">
            <div className="space-y-1.5">
              {editHours.map((h, i) => (
                <div key={i} className="flex items-center gap-2">
                  <span className="text-xs text-gray-600 w-14">{DAY_NAMES[h.dayOfWeek]}</span>
                  ...
                  {!h.closed ? (
                    <>
                      <input type="time" value={h.open}
                        ...
                        className="border border-gray-200 rounded px-1.5 py-1 text-xs bg-white focus:outline-none focus:ring-1 focus:ring-[#25D366]" />
            ...
            <div className="mt-3 pt-3 border-t border-gray-200 flex items-center gap-3">
              <span className="text-xs text-gray-600 whitespace-nowrap">Duracao media da refeicao:</span>
              <div className="flex items-center gap-1">
                <input type="number" min={30} max={240} step={15} value={mealDuration}
                  ...
                  className="w-16 border border-gray-200 rounded px-2 py-1 text-xs text-center bg-white focus:outline-none focus:ring-1 focus:ring-[#25D366]" />
                <span className="text-xs text-gray-400">min</span>
              </div>
              <span className="text-[11px] text-gray-400">(Mesas ficam livres automaticamente apos esse tempo)</span>
            </div>
            ...
            {savedHours && <div className="mt-2 bg-green-50 border border-green-200 text-green-800 rounded-lg p-2 text-xs">Horarios salvos!</div>}
          </div>
        </section>
      )}
      ...
      {isPlanB && (
        <section className="mb-8">
          <h2 className="text-base font-bold text-gray-900 mb-3 flex items-center gap-2">
            <span className="w-5 h-5 bg-[#25D366] rounded-full flex items-center justify-center text-white text-xs font-bold">5</span>
            Mesas e Layout
          </h2>
          <TableMapEditor />
        </section>
      ...
      <section className="mb-8">
        <h2 className="text-base font-bold text-gray-900 mb-3 flex items-center gap-2">
          <span className="w-5 h-5 bg-[#25D366] rounded-full flex items-center justify-center text-white text-xs font-bold">6</span>
          Seu Plano
        </h2>
          ...
          {PLANS.map(plan => {
            ...
            return (
              <div key={plan.key} className={`relative border rounded-xl p-4 transition-all ${
                ...
                {plan.popular && (
                  <span className="absolute -top-2.5 left-1/2 -translate-x-1/2 bg-[#25D366] text-white text-[11px] font-bold px-2.5 py-0.5 rounded-full">
                    MAIS POPULAR
                  </span>
                )}
                {isCurrentPlan && (
                  <span className="absolute top-3 right-3 bg-[#25D366] text-white text-[11px] font-bold px-2 py-0.5 rounded-full">
                    ATUAL
                  </span>
                )}
                ...
                <ul className="space-y-1.5 mb-3">
                  ...
                  {plan.notIncluded.map((f, i) => (
                    <li key={`no-${i}`} className="flex items-start gap-1.5 text-xs text-gray-400">
                ...
                {isCurrentPlan ? (
                  <div className="text-center text-xs text-[#25D366] font-semibold py-1.5">
                    Plano ativo
                  </div>
                ) : (
                  <button
      ...
      <div className="border-t border-gray-200 pt-4">
        <p className="text-xs text-gray-400">Restaurant ID: {restaurant.id}</p>
      </div>
    </div>
  )
}
