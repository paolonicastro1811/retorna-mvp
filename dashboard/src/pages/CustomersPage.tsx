    ...
    <div>
      ...
      <div className="flex gap-2 mb-4">
        <input
          ...
          className="flex-1 border border-gray-300 rounded-lg px-3 py-2 text-base focus:outline-none focus:ring-2 focus:ring-[#25D366]"
        />
        <select
          value={filter}
          onChange={e => setFilter(e.target.value)}
          className="border border-gray-300 rounded-lg px-3 py-2 text-base focus:outline-none focus:ring-2 focus:ring-[#25D366]"
        >
      ...
      {filtered.length === 0 ? (
        <div className="text-center py-16 text-gray-400 text-base">Nenhum cliente encontrado</div>
      ) : (
                  ...
                  <td className="px-4 py-2 text-right">
                    {editingId === c.id ? (
                      <input
                        ...
                        disabled={saving}
                        className="w-20 text-right border border-[#25D366] rounded px-1.5 py-0.5 text-base focus:outline-none focus:ring-1 focus:ring-[#25D366]"
                        placeholder="0.00"
                      />
                    ) : (
                      <span
      ...
      <p className="text-sm text-gray-400 mt-3">{filtered.length} cliente(s)</p>
          ...
          <div className="bg-white rounded-xl shadow-xl max-w-sm w-full p-5">
            ...
            <div className="flex gap-2">
              <button
                ...
                className="flex-1 border border-gray-300 text-gray-700 py-2 rounded-lg text-base font-semibold hover:bg-gray-50"
              >
                Cancelar
              </button>
              <button
                onClick={() => doToggle(confirmCustomer, 'active')}
                className="flex-1 bg-yellow-500 text-white py-2 rounded-lg text-base font-semibold hover:bg-yellow-600"
              >
                Ativar mesmo assim
              </button>
            </div>
