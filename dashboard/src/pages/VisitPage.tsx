  ...
  if (result) {
    ...
    return (
      <div className="max-w-sm">
        <h1 className="text-xl font-bold text-gray-900 mb-4">Registrar Visita</h1>

        {/* Success card */}
        <div className="bg-green-50 border border-green-200 rounded-xl p-4 mb-4">
          <div className="flex items-center gap-2 mb-3">
            <div className="w-8 h-8 bg-[#25D366] rounded-full flex items-center justify-center text-white text-base">✓</div>
            <div>
              <p className="text-base font-bold text-green-900">Visita registrada!</p>
              <p className="text-sm text-green-700">{result.customer.name || phone}</p>
            </div>
          </div>
          ...
          {lf && (
            <div className="space-y-2 mt-3 pt-3 border-t border-green-200">
              ...
              <div className="flex items-center justify-between">
                <span className="text-sm text-green-800">Nivel atual</span>
                <span className={`text-sm font-semibold px-2 py-0.5 rounded-full ${TIER_COLOR[lf.tier] || TIER_COLOR.novo}`}>
                  {TIER_EMOJI[lf.tier] || ''} {TIER_LABEL[lf.tier] || lf.tier}
                </span>
              </div>

              <div className="flex items-center justify-between">
                <span className="text-sm text-green-800">Total de visitas</span>
                <span className="text-sm font-bold text-green-900">{lf.totalVisits}</span>
              </div>
              ...
              {lf.nextTier ? (
                ...
                <p className="text-xs text-green-700 font-semibold">🥇 Nivel maximo atingido!</p>
              )}

              {/* Streak */}
              {lf.streakTarget > 0 && (
                <div className="flex items-center justify-between mt-1">
                  <span className="text-sm text-green-800">Sequencia semanal</span>
        ...
        <button
          onClick={reset}
          className="w-full bg-[#25D366] text-white py-2 rounded-lg font-semibold text-sm hover:bg-[#1DA851] transition-colors"
        >
          ...
          {showDropdown && results.length > 0 && (
            <div className="absolute z-10 w-full mt-1 bg-white border border-gray-200 rounded-lg shadow-lg overflow-hidden">
              {results.map(c => (
                ...
                >
                  <div>
                    <p className="text-sm font-medium text-gray-900">{c.name || 'Sem nome'}</p>
                    <p className="text-xs text-gray-500">{c.phone}</p>
                  </div>
                  <span className={`text-xs px-1.5 py-0.5 rounded-full ${TIER_COLOR[c.tier] || TIER_COLOR.novo}`}>
        ...
        {selected && (
          <div className="bg-[#1a1a2e] rounded-lg p-3">
            <div className="flex items-center justify-between mb-2">
              <p className="text-sm font-bold text-white">{selected.name || 'Sem nome'}</p>
              <span className={`text-xs px-1.5 py-0.5 rounded-full ${TIER_COLOR[selected.tier]}`}>
            ...
            <div className="grid grid-cols-3 gap-2">
              <div>
                ...
                <p className="text-sm font-bold text-white">{selected.totalVisits}</p>
              </div>
              <div>
                <p className="text-xs text-gray-400">Sequencia</p>
                <p className="text-sm font-bold text-white">🔥 {selected.currentStreak}</p>
              </div>
              <div>
    ...
    <div className="mt-6 border-t border-gray-200 pt-4">
      <div className="bg-[#1a1a2e] rounded-xl p-4">
        <div className="flex items-center gap-2 mb-2">
          ...
          <h3 className="text-sm font-bold text-white">Quer automatizar?</h3>
        </div>
        ...
        <a
          ...
          className="inline-flex items-center gap-1.5 bg-[#25D366] text-white px-3 py-1.5 rounded-lg text-xs font-semibold hover:bg-[#1DA851] transition-colors"
        >
